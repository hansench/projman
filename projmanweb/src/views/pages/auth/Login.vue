...
const schema = Yup.object().shape({
    userName: Yup.string()
        ...
        .email((data) => `${data.label} is not valid`),
    password: Yup.string()
        .label('Password')
        .required((data) => `${data.label} is required`)
});
            ...
            <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                    <div class="text-center mb-8">
                        ...
                        <div class="text-surface-900 dark:text-surface-0 text-3xl font-medium mb-4">Welcome to ProjMan!</div>
                        <span class="text-muted-color font-medium">Sign in to continue</span>
                    </div>

                    <form @submit="onSubmit" class="flex flex-col w-full md:w-[30rem] mb-8">
                        <div class="mb-4">
                            <label for="userName" class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">Email</label>
                            <InputText id="userName" type="text" placeholder="Email address" fluid v-model="userName" autofocus />
                            <Message v-if="errors.userName" severity="error" size="small" variant="simple">{{ errors.userName }}</Message>
                        </div>

                        <div class="mb-4">
                            <label for="password" class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">Password</label>
                            <Password id="password" v-model="password" placeholder="Password" :toggleMask="true" class="mb-4" fluid :feedback="false" :class="{ 'p-invalid': errors.password }"></Password>
                        ...
                        <Button type="submit" label="Sign In" class="w-full" :disabled="authStore.loading" :loading="authStore.loading"></Button>
                    </form>
                </div>
            </div>
        </div>
